	...
	pickers
		.new({
			...
			attach_mappings = function(prompt_bufnr, map)
				-- Map 'd' in normal mode to delete
				map("n", "d", function()
					local selection = action_state.get_selected_entry()
					if selection.value == "newProject" then
						vim.api.nvim_err_writeln("Cannot delete 'Create new project' entry.")
						return
					end
					-- Confirm deletion
					local confirm = vim.fn.confirm("Delete project '" .. selection.display .. "'?", "&Yes\n&No", 2)
					if confirm == 1 then
						-- Close the picker
						actions.close(prompt_bufnr)
						-- Delete the project
						tmux.delete_project(selection.value, workspace)
					end
				end)
			...
			attach_mappings = function(prompt_bufnr, map)
				...
				map("n", "d", function()
					...
					if confirm == 1 then
						...
						tmux.kill_session(selection.value)
					end
				end)
